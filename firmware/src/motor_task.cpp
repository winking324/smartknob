...
void MotorTask::run() {
  ...
  // Tune zero offset to the specific hardware (motor + mounted magnetic
  // sensor). SimpleFOC is supposed to be able to determine this automatically
  // (if you omit params to initFOC), but it seems to have a bug (or I've
  // misconfigured it) that gets both the offset and direction very wrong! So
...
// motor.LPF_angle = LowPassFilter(0.05);
  ...
  // because SimpleFOC studio supports updating them easily over serial for
  // tuning.
  ...
  if (calibrate) {
    ...
    motor.move(a);
    // Serial.println("Did motor turn counterclockwise? Press Y to continue,
    // otherwise change motor wiring and restart"); while (Serial.read() != 'Y')
    // {
    //     delay(10);
    // }
    ...
    // Rotate many electrical revolutions and measure mechanical angle traveled,
    ...
    for (; a < destination1; a += 0.4) {
      ...
      float measured_electrical_angle = _normalizeAngle(
          (float)(motor.sensor_direction * measured_pole_pairs) *
  ...
  while (1) {
    ...
    if (xQueueReceive(queue_, &command, 0) == pdTRUE) {
      switch (command.command_type) {
        case CommandType::CONFIG: {
          ...
          // Update derivative factor of torque controller based on detent
          // width. If the D factor is large on coarse detents, the motor ends
          // up making noise because the P&D factors amplify the noise from the
          // sensor. This is a piecewise linear function so that fine detents
          // (small width) get a higher D factor and coarse detents get a small
          // D factor. Fine detents need a nonzero D factor to artificially
          // create "clicks" each time a new value is reached (the P factor is
          // small for fine detents due to the smaller angular errors, and the
          // existing P factor doesn't work well for very small angle changes
          // (easy to get runaway due to sensor noise & lag)).
          ...
          const float derivative_lower_strength =
              config.detent_strength_unit * 0.08;
          const float derivative_upper_strength =
              config.detent_strength_unit * 0.02;
          const float derivative_position_width_lower = radians(3);
          const float derivative_position_width_upper = radians(8);
          const float raw =
              derivative_lower_strength +
              (derivative_upper_strength - derivative_lower_strength) /
                  (derivative_position_width_upper -
                   derivative_position_width_lower) *
                  (config.position_width_radians -
                   derivative_position_width_lower);
          motor.PID_velocity.D = CLAMP(
              raw, min(derivative_lower_strength, derivative_upper_strength),
    ...
    idle_check_velocity_ewma =
        motor.shaft_velocity * IDLE_VELOCITY_EWMA_ALPHA +
    ...
    // If we are not moving and we're close to the center (but not exactly
    // there), slowly adjust the centerpoint to match the current position
    if (last_idle_start > 0 &&
        millis() - last_idle_start > IDLE_CORRECTION_DELAY_MILLIS &&
        fabsf(motor.shaft_angle - current_detent_center) <
            IDLE_CORRECTION_MAX_ANGLE_RAD) {
      current_detent_center =
          motor.shaft_angle * IDLE_CORRECTION_RATE_ALPHA +
...
#endif
    if (angle_to_detent_center >
            config.position_width_radians * config.snap_point &&
      ...
      config.position--;
    } else if (angle_to_detent_center <
                   -config.position_width_radians * config.snap_point &&
    ...
    float dead_zone_adjustment =
        CLAMP(angle_to_detent_center,
              fmaxf(-config.position_width_radians * DEAD_ZONE_DETENT_PERCENT,
                    -DEAD_ZONE_RAD),
              fminf(config.position_width_radians * DEAD_ZONE_DETENT_PERCENT,
                    DEAD_ZONE_RAD));

    bool out_of_bounds =
        config.num_positions > 0 &&
        ((angle_to_detent_center > 0 && config.position == 0) ||
         (angle_to_detent_center < 0 &&
          config.position == config.num_positions - 1));
    motor.PID_velocity.limit = 10;  // out_of_bounds ? 10 : 3;
    motor.PID_velocity.P = out_of_bounds ? config.endstop_strength_unit * 4
                                         : config.detent_strength_unit * 4;
    ...
    if (fabsf(motor.shaft_velocity) > 60) {
      ...
      float torque =
          motor.PID_velocity(-angle_to_detent_center + dead_zone_adjustment);
#if SK_INVERT_ROTATION
      torque = -torque;
#endif
      motor.move(torque);
...
void MotorTask::setConfig(const KnobConfig& config) {
  Command command = {.command_type = CommandType::CONFIG,
                     .data = {
                         .config = config,
                     }};
...
void MotorTask::playHaptic(bool press) {
  Command command = {.command_type = CommandType::HAPTIC,
                     .data = {
                         .haptic =
                             {
                                 .press = press,
