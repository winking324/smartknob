...
static KnobConfig configs[] = {
    ...
    },
    {
        ...
        1,
        1,
        0.55,  // Note the snap point is slightly past the midpoint (0.5);
               // compare to normal detents which use a snap point *past* the
               // next value (i.e. > 1)
        "On/off\nStrong detent",
    },
    {
...
InterfaceTask::InterfaceTask(const uint8_t task_core, MotorTask& motor_task,
...
void InterfaceTask::run() {
  ...
  while (1) {
...
#if SK_STRAIN
    // TODO: calibrate and track (long term moving average) zero point (lower);
    // allow calibration of set point offset
    ...
    if (scale.wait_ready_timeout(100)) {
      ...
      if (reading >= lower - (upper - lower) &&
          reading < upper + (upper - lower) * 2) {
...
// TODO: brightness scale factor should be configurable (depends on reflectivity
// of surface)
#if SK_ALS
    brightness =
        (uint16_t)CLAMP(lux_avg * 13000, (float)1280, (float)UINT16_MAX);
#endif
...
void InterfaceTask::handleEvent(AceButton* button, uint8_t event_type,
